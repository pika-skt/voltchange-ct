...
from runtime.tokenizer import load_tokenizer
...
def raw_feature_vector(
    ...
    *,
    ...
    """Build dense regex features plus signed tokenizer unigram/bigram bins."""
...
def predict_episode(
    ...
    *,
    ...
    raw = raw_feature_vector(episode, artifact.hash_bins, tokenize=tokenize)
    standardized = tuple(
        (value - mean) / scale
...
def fill_ax31_upgrades(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "선택과 예측 배열은 같은 길이의 비어 있지 않은 배열이어야 합니다."
        )
    ...
    def choose(penalty: float) -> Tuple[Tuple[str, ...], float]:
        ...
        total = math.fsum(
            costs[model_id] for costs, model_id in zip(predicted_costs, filled)
        )
...
def make_hash_regex_submission(
    ...
    *,
    ...
    selected_tokenize = tokenize or load_tokenizer()
    predictions = [
        predict_episode(episode, artifact, tokenize=selected_tokenize)
